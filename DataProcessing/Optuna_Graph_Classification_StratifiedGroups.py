...
class GNN3(torch.nn.Module):
    def __init__(self, hidden_channels: int, num_node_features: int, num_classes: int, num_heads: int,
                 normalise_graph: bool = True, gnn_task: str = "graph_classification"):
        super().__init__()
        torch.manual_seed(12345)
        self.task = gnn_task
        self.normalise = normalise_graph
...
class Processing:
    def __init__(self, corpus: str, aoi_type: str, num_node_features: int, which_data_type: str,
        ...
        self.scanpath_df_all, self.scanpath_df_agree, self.scanpath_df_topical = self.load_data()
    ...
    def prepare_data(self, df: pd.DataFrame):
        ...
        for ind in df.index:
            ...
            labels_for_split_list.append(label)
    ...
    def load_data(self):
        ...
        scanpath_all = pd.read_csv(
            f"../Data/Graph_Data/gREL_{self.AOI_Type}_{self.Number_Of_Node_Features_File_Name}_Features.csv")
        scanpath_agree = scanpath_all.loc[scanpath_all["label"] == scanpath_all["system_label"]]
        scanpath_agree = scanpath_agree.loc[scanpath_agree["gREL_label"] != "t"].reset_index(drop=True)
        scanpath_topical = scanpath_all.loc[scanpath_all["gREL_label"] == "t"].reset_index(drop=True)
    ...
    def process_scanpaths(self, df: pd.DataFrame, data_type: str):
        ...
        for fold_number, (training_val_index, testing_index) in enumerate(
            ...
            df_test_results.to_csv(f'../Results/GNN/'
    ...
    def run(self):
        ...
        elif self.which_data_type == "Topical":
            self.process_scanpaths(df=self.scanpath_df_topical, data_type="topical")
...
if __name__ == '__main__':
    ...
    logging.basicConfig()
    GPU = 0
    for Data_Type in ["All", "Agree", "Topical"]:
        for AOI_Type in ["Paragraph", "Line", "Cluster_Affinity", "Quartile"]:
            for Model in ["GNN1", "GNN2", "GNN3"]:
                for Number_Of_Node_Features in [2, 17]:
                    processes = Processing(corpus="gREL",
                                           aoi_type=AOI_Type,
                    ...
                    print(f"gREL {Data_Type}, {AOI_Type}, {Model}, {Number_Of_Node_Features} Done")
